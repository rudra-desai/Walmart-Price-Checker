...
def get_Price_by_link(urlLink):
    ...
    nameLine = str(
        soup.findAll(
            "h2",
            {"class": "prod-ProductTitle no-margin font-normal heading-b"}))
    name = (nameLine[nameLine.index('content=') + 9:nameLine.index('">')])
    priceLine = str(
        soup.findAll("span", {
            "class":
            re.compile("^price display-inline-block arrange-fit price")
        }))
...
def get_Price_by_title(search):
    link = (
        "https://www.walmart.com/search/?cat_id=0&grid=true&page=1&query=%s" %
        search)
    uClient = uReq(link)
    page = uClient.read()
    uClient.close()
    links_with_text = []
    soup = BeautifulSoup(page, "html.parser")
    nameLine = soup.findAll(
        'a', attrs={'class': "product-title-link line-clamp line-clamp-2"})
    allProducts = {}
    prodcutName = ""
    counter = 1
    for links in soup.findAll('a', attrs={'href': re.compile("^/ip")}):
        ...
        prodcutName = (
            link_with_name[link_with_name.index("ip/") +
                           3:find_nth_occurence(link_with_name, "/", 3)])
    ...
    if [value for key, value in allProducts.items()]:
        for key2 in allProducts:
            if whichOne in key2.lower():
                ...
                name = key2
                break
    return (get_Price_by_link(productLink)[0],
            get_Price_by_link(productLink)[1],
            get_Price_by_link(productLink)[2])
...
def sendEmail(msg, productName, url):
    ...
    body = "Price went down, The price is now: $" + str(
        msg) + " for " + productName + "\n" + str(url)
...
userSelection = int(
    input(
        "Enter 1 to receive an email when the price drops, Enter 2 to search the price by title or 3 to search the price by link: "
    ))

if userSelection == 1:
    product = int(
        input(
            "Enter 1 to retrieve product price by title or 2 to retrieve product price by link: "
        ))
    if (product == 1):
        search = input("Search by Title: ").replace(" ", "+")
        productInfo = get_Price_by_title(search)
        print("\nThe current price of " + productInfo[0] + " is: $" +
              str(productInfo[1]))
        desiredPrice = int(
            input(
                "At what price would you like to be notified about the price drop?: "
            ))
        while (True):
